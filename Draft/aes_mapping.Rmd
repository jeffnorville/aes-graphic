...
simplinks <- read.csv("../simplifylinks.csv", sep=";", header=TRUE, stringsAsFactors = FALSE)
...
vis.simplinks$dashes <- simplinks$type #c(TRUE, FALSE)[aeslinks$type]
...
vis.ccnodes <- subset(aesnodes, com_solution == c("Crop choice"))
...
vis.cclinks <- subset(aeslinks, group == "Crop choice")
visNetwork(vis.ccnodes, vis.cclinks) 

net <- graph_from_data_frame(d=vis.cclinks, vertices=vis.ccnodes, directed=TRUE)
# plot(net, layout = layout_with_lgl, label = aesnodes$short.definition)
plot(net)
